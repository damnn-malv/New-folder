    ...
    <div className="rpt-card rpt-section">
      <div className="rpt-card-header">
          ...
          <div className="rpt-batch-toggle">
            {[["all", "All"], ["batch1", "Batch 1"], ["batch2", "Batch 2"]].map(([val, lbl]) => (
              <button
                ...
                onClick={() => { setFilters((f) => ({ ...f, batch: val })); setShowAllCollections(false); }}
              >
                {lbl}
              </button>
            ))}
        ...
        <div className="rpt-card-header-actions">
          ...
          <button className="rpt-btn-export rpt-btn-export--navy" onClick={handleExportPDF}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
      ...
      <DataTable
        columns={["Date & Time", "Batch", "Ticket ID", "Driver", "Vehicle", "Route", "Amount"]}
        data={visibleCollections}
        rowRenderer={(r, idx, { rowClass, cellClass }) => (
          <tr key={r.id} className={rowClass}>
            <td className={cellClass}>{r.issued_at}</td>
            <td className={cellClass}>
              <span className={`rpt-batch-pill ${r.batch === "Batch 1" ? "rpt-batch-pill--b1" : "rpt-batch-pill--b2"}`}>
                {r.batch}
              </span>
            </td>
            ...
            <td className={`${cellClass} rpt-amount`}>{peso((r.ticket_count || 1) * TICKET_FEE)}</td>
          </tr>
        )}
      />
      ...
      {filteredCollections.length > 5 && (
        <div className="rpt-show-more">
          <button className="rpt-btn rpt-btn--secondary" onClick={() => setShowAllCollections((v) => !v)}>
